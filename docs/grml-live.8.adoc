...
grml-live image-create [--boot-options DEFAULT_BOOTOPTIONS] [-c CLASS1,CLASS2]
          ...
          [-I CHROOT_INSTALL_DIRECTORY] [--on-error-shell] [-r RELEASE_NAME] [-R] [-s SUITE]
          [--secure-boot] [-v VERSION_NUMBER]
          [--bootstrap-mirror-url BOOTSTRAP_MIRROR_URL | -w DATE]
          OUTPUT_DIRECTORY
...
On a build failure, start an interactive shell inside the Linux user namespace.
The shell can be used to inspect the current state of the build chroot, before it is cleaned up by grml-live.
...
To get a Grml-based Live-CD based on Debian testing, created in `./my-grml` as output directory just run:
  ...
  $ grml-live image-create ./my-grml
...
system:
  ...
  $ grml-live image-create -s testing -c GRML_SMALL /srv/grml-live
...
`grml-live image-update -c GRML_SMALL,FOOBAR -D /path/to/config ...`.
...
Invoke the ISO build process:
  ...
  $ ./grml-live image-create -c GRML_FULL ./my-grml
...
The resulting ISO can then be found in `./my-grml/grml_isos/`.
...
own. Usage example:
  ...
  grml-live image-create -I chroot_install ... ./my-grml
...
And pass it as an **additional** class to grml-live:
  ...
  $ grml-live image-create -c GRML_SMALL,MIKA ./my-grml
...
Usage example:
    ...
    $ ./grml-live image-create -D "$PWD/config" -s sid -c GRML_FULL ./my-grml

...
-v <version_number>] [-w <date>] [-bBFqRu]
...
Invoke the ISO build process:
  ...
  $ sudo ./grml-live -C ./etc/grml/grml-live.local
...
The variable `GRML_FAI_CONFIG` should be set so `grml-live` can find the
class-based configuration files.
This is especially important to avoid re-using the config files of a previously
installed `grml-live` Debian package.

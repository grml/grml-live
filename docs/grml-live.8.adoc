...
grml-live [-h] [--boot-options DEFAULT_BOOTOPTIONS] [-c CLASS1,CLASS2] [--distri-info DISTRI_INFO]
          [--distri-name DISTRI_NAME] [--hostname HOSTNAME] [--username USERNAME] [-d DATE] [-D CONFIGDIR]
          [-e EXTRACT_ISO_NAME] [-F] [-g GRML_NAME] [-i ISO_NAME] [-I CHROOT_INSTALL_DIRECTORY]
          [-o OUTPUT_DIRECTORY] [-r RELEASE_NAME] [-R] [-s SUITE] [--secure-boot] [-v VERSION_NUMBER]
          [--bootstrap-mirror-url BOOTSTRAP_MIRROR_URL | -w DATE] [-b | -B | -u | -q]
...
This option is useful for remastering, in combination with -b or -u.
...
- next to `grml_isos`, `grml_chroot`, etc.
...
Usage example:
    ...
    $ ./grml-live -D "$PWD/config"  -s sid -c GRML_FULL
